...
class AppointmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $appointments = Appointment::filterMultiple($type_date,$start_date,$end_date,$state_pay,$state,$specie,$search_pets,$search_vets,$user)->orderBy("id","desc")->paginate(10);
    ...
    public function filter(Request $request) {
        ...
        // 1.-Obtiene el nombre del dia de la fecha que hemos seleccionado
        date_default_timezone_set('America/Lima');
        Carbon::setLocale('es');
        $dayName = Carbon::parse($date_appointment)->dayName;

        // 2.- Obtiene la lista de veterinarios que atienden ese dia (Schedule_days)
        $veterinarie_days = VeterinarieScheduleDay::where("day","ilike","%".$dayName."%")->orderBy("veterinarie_id","asc")->get();

        // 3.- Obtiene los segmentos de tiempos o hora del dia de atención
        ...
        foreach ($veterinarie_days as $key => $veterinarie_day) {
            ...
            foreach ($segment_time_joins as $segment_time_join) {
                //SE OBTIENE LA FECHA DE LA CITA , EL VETERINARIO Y EL SEGMENTO DE TIEMPO O HORA PARA NO REGISTRAR
                //UNA MISMA CITA DUPLICADA
                ...
                if(!$check){
                    $check = Vaccination::whereDate("date_appointment",$date_appointment)
                                        ->where("state","<>",2)
                                     ->where("veterinarie_id",$veterinarie_day->veterinarie_id)
                                     ->whereHas("schedules",function ($q) use($segment_time_join){
                                        $q->where("veterinarie_schedule_hour_id",$segment_time_join->veterinarie_schedule_hour_id);
                    ...
                    if(!$check){
                        $check = Surgerie::whereDate("date_appointment",$date_appointment)
                                        ->where("state","<>",2)
                                     ->where("veterinarie_id",$veterinarie_day->veterinarie_id)
                                     ->whereHas("schedules",function ($q) use($segment_time_join){
                                        $q->where("veterinarie_schedule_hour_id",$segment_time_join->veterinarie_schedule_hour_id);
    ...
    public function searchPets($search) {
        $pets = Pet::whereHas('owner', function($q) use ($search) {
            $q->where(function ($query) use ($search) {
                $query->where('pets.name', 'ILIKE', "%{$search}%")
                      ->orWhere('owners.names', 'ILIKE', "%{$search}%")
                      ->orWhere('owners.surnames', 'ILIKE', "%{$search}%")
                      ->orWhere('owners.phone', 'ILIKE', "%{$search}%")
                      ->orWhere('owners.n_document', 'ILIKE', "%{$search}%");
            });
        })
        ->limit(5)
        ->get();
        ...
        return response()->json([
            "pets" => $pets->map(function($pet) {
                return [
                    ...
                    "owner" => [
                        "id" => $pet->owner->id,
                        "names" => $pet->owner->names,
                        "surnames" => $pet->owner->surnames,
                        "phone" => $pet->owner->phone,
                        "type_document" => $pet->owner->type_document,
